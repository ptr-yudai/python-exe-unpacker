...
class PyInstArchive:
    ...
    def open(self):
        try:
            ...
            logger.error('Could not open `{}`'.format(self.filePath))
            return False
        return True
    ...
    def checkFile(self):
        ...
        logger.error('Unsupported pyinstaller version or not a pyinstaller archive')
        return False
    ...
    def getCArchiveInfo(self):
        try:
            ...
            logger.error('The file is not a pyinstaller archive')
            return False
        ...
        logger.info('Length of package: {0} bytes'.format(self.overlaySize))
        return True
    ...
    def parseTOC(self):
        ...
        while parsedLen < self.tableOfContentsSize:
            ...
            if len(name) == 0:
                name = str(uniquename())
                logger.warn('Found an unamed file in CArchive. Using random name `{}`'.format(name))
            ...
            parsedLen += entrySize
        logger.info('Found {} files in CArchive'.format(len(self.tocList)))
    ...
    def extractFiles(self, extractionDir):
        ...
        for entry in self.tocList:
            ...
            if entry.typeCmprsData == b's':
            	logger.info('Possible entry point: {0}'.format(entry.name))
    ...
    def _extractPyz(self, name):
        ...
        with open(name, 'rb') as f:
            ...
            if imp.get_magic() != pycHeader:
                logger.warn('The script is running in a different python version than the one used to build the executable')
                logger.warn('Run this script in Python{} to prevent extraction errors(if any) during unmarshalling'.format(self.pyver))
            ...
            try:
                ...
                logger.error('Unmarshalling FAILED. Cannot extract `{}`. Extracting remaining files.'.format(name))
                return

            logger.info('Found {} files in PYZ archive'.format(len(toc)))
            ...
            for key in toc.keys():
                ...
                try:
                    ...
                    logger.error('Failed to decompress `{}`, probably encrypted. Extracting as is.'.format(fileName))
